...
## Answer 1
There are 7,014 incumbent candidates. 
```{r}
library(tidyverse)
library(dplyr)
library(ggplot2)
fb_ads <- read.csv("data/fb_ads.csv") 
incumbency_table <- table(fb_ads$incumbency, useNA= "always")
incumbency_table
```
...
## Answer 2
A democratic incumbent candidate sponsored the most promoting ads on average. 
```{r}
filtered_fb_ads <- fb_ads |>
  filter(office %in% c("US House", "US Senate"))|>
   group_by(party, incumbency)|>
  summarize(promote_prop = mean(ad_tone_promote, na.rm = TRUE)) 
party_incumbent_promote <- filtered_fb_ads
party_incumbent_promote
knitr::kable(party_incumbent_promote, digits = 3, col.names = c("Party", "Incumbency Status", "Average Promotion Proportion")) 
```
...
## Answer 3
The histogram is shaped with the majority of the data at the value of zero on the x-axis. The majority of the House candidates did not have more than 10 million impressions on Facebook. 
```{r}
fb_ads <- fb_ads |>
  filter(office == "US House") |> 
  mutate(impressions_millions = impressions / 1000000)

plot_q3 <- ggplot(fb_ads, mapping = aes(x=impressions_millions))+
           labs(x = "Facebook Impressions in Millions", y= "Count of House Candidates")+
  geom_histogram()
...
fb_top_plot <- ggplot(fb_ads, mapping = aes(x=impressions_millions,
                                            y=cand_name_last)) + 
                                              
   geom_bar(stat = "identity", width = 0.50, aes(fill = party), na.rm = TRUE) +
   labs(title = "Top 15 House Candidate Facebook Ad Impressions, 2017-18", x = "Facebook Ad Impressions (in millions)", y = "Candidates by Last Name") + 
